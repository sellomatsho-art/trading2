            ...
            try:
                ...
                for market in markets:
                    try:
                        ...
                        mid = market.get('id')
                        name = market.get('question', 'Market')[:40]
                        ...
                        if len(self.price_history[mid]) >= 3 and len(self.positions) < 5:
                            ...
                            if vol > 0.1 and mid not in self.positions:
                                ...
                                print(f"[OPEN] {name[:30]} vol:{vol:.3f}%")
                        ...
                        if mid in self.positions:
                            ...
                            if roi > 1 or roi < -0.5:
                                ...
                                print(f"[CLOSE] {pos['name'][:30]} ROI:{roi:.2f}%")
                    except:
                        continue
                
                if scan % 3 == 0:
                    pnl = sum([t['pnl'] for t in self.trades]) if self.trades else 0
                    print(f"Scans:{scan} Trades:{len(self.trades)} Open:{len(self.positions)} PnL:${pnl:.2f}")
